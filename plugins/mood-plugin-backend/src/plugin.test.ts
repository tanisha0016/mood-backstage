...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [moodPluginPlugin],
    });

    await request(server).get('/api/mood-plugin/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/mood-plugin/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/mood-plugin/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/mood-plugin/todos/${createdTodoItem.id}`)
